...
class DataGenerator():
  def __init__(self, datapath, itempath):
    ...
    '''
    self.data  = self.load_datas(datapath, itempath) # 加载数据
  ...
  def load_datas(self, datapath, itempath):
    ...
    return data.merge(movie_titles,on='itemId', how='left') # data - userid, itemid, ranting, timestamp, and moviename
  ...
  def gen_histo(self):
    ...
    for i, u in enumerate(self.users):
      temp = self.data[self.data['userId'] == u] # 所有数据
      temp = temp.sort_values('timestamp').reset_index()
      temp.drop('index', axis=1, inplace=True)
      historic_users.append(temp)
    return historic_users # sort the history of user , historic_users - key is user, values is the ratings this user given
  ...
  def sample_histo(self, user_histo, action_ratio=0.8, max_samp_by_user=5,  max_state=100, max_action=50, nb_states=[], nb_actions=[]):
    ...
    n = len(user_histo) # 用户有多少打分记录
    sep = int(action_ratio * n) # sep = 记录的 80% （action_ration = 0.8）
    nb_sample = random.randint(1, max_samp_by_user) #  max_samp_by_user=5 采样次数么？
    if not nb_states: # nb_state 为空 每次采样的state数？
      nb_states = [min(random.randint(1, sep), max_state) for i in range(nb_sample)]
    if not nb_actions: # nb_action 为空 每次采样的action数？
    ...
    # 每个用户采样 nb_states次; states 为状态; actions为take action的次数
    for i in range(len(nb_states)):
      sample_states = user_histo.iloc[0:sep].sample(nb_states[i]) # 前80%为state , 采样个数为随机，但是有max_state
      sample_actions = user_histo.iloc[-(n - sep):].sample(nb_actions[i]) #后20%位take action 采样同state，同样有max_action
      ...
      for j in range(nb_states[i]):
        ...
        # FORMAT STATE
        state = str(row.loc['itemId']) + '&' + str(row.loc['rating']) # format是 movieid &  rating
        sample_state.append(state)
      
      for j in range(nb_actions[i]):
        row    = sample_actions.iloc[j]
        # FORMAT ACTION
        action = str(row.loc['itemId']) + '&' + str(row.loc['rating']) # action movieid & action
  ...
  def gen_train_test(self, test_ratio, seed=None): # test ration ???
    '''
    ...
    seed       :  float
                  Seed on the shuffle
    '''
    n = len(self.histo) # the number of users
    ...
    self.train = self.histo[:int((test_ratio * n))] # the trainset is less, when the test_tation = 0.8
    self.test  = self.histo[int((test_ratio * n)):]
    self.user_train = [h.iloc[0,0] for h in self.train] # traini users
  ...
  def write_csv(self, filename, histo_to_write, delimiter=';', action_ratio=0.8, max_samp_by_user=5, max_state=100, max_action=50, nb_states=[], nb_actions=[]):
    ...
    columns : state;action_reward;n_state # state is what, action reward is what? n_state is what
    ...
    with open(filename, mode='w') as file:
      ...
      for user_histo in histo_to_write:
        # user history sorted by the timestamps
        #                                          action_ratio=0.8, max_samp_by_user=5, max_state=100, max_action=50, nb_states=[], nb_actions=[]
